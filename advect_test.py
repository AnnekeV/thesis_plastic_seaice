...
def make_glorys_fieldset(year):
    ...
    ifiles = sorted( glob.glob(datadir+ "GLOBAL_REANALYSIS_PHY_001_030-TDS_????????_uv_uvice_con_thick.nc") )[2000:2100]
...
def get_particle_set(fieldset, start_date):
    ...
    lon = np.repeat(-169.0, npart)
    lat = np.repeat(66.0, npart)
                
    time = np.arange(np.datetime64(start_date), np.datetime64(start_date) + np.timedelta64(npart, 'D'))
...
#         fieldset.computeTimeChunk(p.time, )
...
def run_experiment(start_date, simdays = 35, custom_kernel = "AdvectionRK4_prob", outputdir = '/home/students/6252699/thesis/parcels2/output/', year = 2014, dt_days=5):
    ...
    print("yes fieldset import works {}".format(datadir))
    '''
    ...
    output_name = outputdir   + "05-06-test-7-bering-everyday_npart_{}_start_{}_simdays_{}_kernel_{}_dtdays_{}".format(npart, start_date,  simdays, custom_kernel, dt_days)
    '''
