...
def make_glorys_fieldset(year):
    ...
    ifiles = sorted( glob.glob("/scratch/AnnekeV/reanalysis_data/" + "GLOBAL_REANALYSIS_PHY_001_030-TDS_????????_uv_uvice_con_thick.nc") )[-3000:]
...
def get_particle_set(fieldset, start_date, kernel):
    ...
    ndays = 365*3
    lon   = np.tile(np.linspace(10,10.5,10), (ndays,1)).flatten()
    lat   = np.repeat(67, ndays*10)
...
def run_experiment(start_date, simdays = 35, custom_kernel = "AdvectionRK4_prob", outputdir = '/home/students/6252699/thesis/parcels2/output/', year = 2014, dt_days=5):
    ...
    output_name = outputdir   + "norway-everyday-05-30_npart_{}_start_{}_simdays_{}_kernel_{}_dtdays_{}_10_at_a_time".format(npart, start_date,  simdays, custom_kernel, dt_days)
...
if __name__ == "__main__":
    ...
    year        = 2011
    dt_days     = 1
    for kernel in ["AdvectionRK4_prob", "AdvectionRK4_ocean"]:
