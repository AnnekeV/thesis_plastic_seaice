...
def make_distance_plots_virtual_vs_observed(drifter_ids, name='', fig_save=False, plot_map_drift=False, method='median'):
    ...
    for drifter in drifter_ids:
        try:
            ...
            if plot_map_drift:
                ...
                ax.plot(one_buoy['lon'], one_buoy['lat'], '--', transform=ccrs.Geodetic(), label="buoy 21",  linewidth=2, color='blue')
                ax.plot(ds_new['lon'][i], ds_new['lat'][i], transform=ccrs.Geodetic(), label ='parcels, sig, {}'.format(i), linewidth=.5, color = 'green')
                ax.plot(ds_old['lon'][i], ds_old['lat'][i],'-.', transform=ccrs.Geodetic(), label ='parcels, sic, {}'.format(i), linewidth=.5, color = 'red')
    ...
    #         print (len(one_buoy)wh)
            ...
            ax_dist.plot(x/day, dists_old, '--', color=colors[count], label='sic/old')
...
#     plt.show()
    ...
    ax_mean.plot(XX, comb_mean_o, '--', label='sic-dependent kernel', color='blue',zorder=2)
    ...
    if fig_save:
        fig_mean.savefig("figures/distance_buoy_parcels/traj_buoy/traj_buoy_{}_total_mean_region_{}_ndrift_{}.png".format(method,name, count))
        fig.savefig("figures/distance_buoy_parcels/traj_buoy/traj_buoy_{}_map_region_{}_ndrift_{}.png".format(method,name, count))
        fig_distances.savefig("figures/distance_buoy_parcels/traj_buoy/traj_buoy_{}_seperate_distances_region_{}_ndrift_{}.png".format(method,name, count))
...
#     ax_dist.set_ylim([0,50])
    ...
    if fig_save:
        fig_mean.savefig("figures/distance_buoy_parcels/within_traj/within_traj_{}_of_mean_region_{}_ndrift_{}.png".format(method, name, count))
        fig.savefig("figures/distance_buoy_parcels/within_traj/within_traj_{}_map_region_{}_ndrift_{}.png".format(method, name, count))
        fig_distances.savefig("figures/distance_buoy_parcels/within_traj/within_traj_{}_seperate_distances_region_{}_ndrift_{}.png".format(method, name, count))
