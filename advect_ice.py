...
def make_glorys_fieldset(year):
    ...
    ifiles = sorted(glob.glob("/scratch/AnnekeV/reanalysis_data/" + "GLOBAL_REANALYSIS_PHY_001_030-TDS_{}????_uv_uvice_con_thick.nc".format(year) )
...
#     lon,lat = grid_per_x_km(xkm=10e4, ylims = [50,90])   # xkm = km between data points
    lon, lat = np.meshgrid(np.arange(-180+1./15,180, 2./15.), np.arange(50+1./15,90, 2./15.))
...
def run_experiment(start_date, simdays = 35, custom_kernel = "AdvectionRK4_prob", outputdir = '/home/students/6252699/thesis/parcels2/output/', year = 2014):
    ...
    output_name = outputdir   + "04-05_npart_{}_start_{}_simdays_{}_kernel_".format(npart, start_date,  simdays) + custom_kernel
...
if __name__ == "__main__":
    ...
    simdays     = 40
    year        = 2014
    output_name = run_experiment(start_date = start_date, simdays = simdays, custom_kernel = kernel, outputdir = '/scratch/AnnekeV/output/11-4-combination-two-runs/', year = year)
